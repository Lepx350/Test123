...
# ── Fallback: bare prompt: field (flat panel format from Step 4) ──
if 'g' not in p:
    bare_prompt = re.search(r'(?<!\w)prompt:\s*"((?:[^"\\]|\\.)*)"', ptext)
    if bare_prompt:
        p['g'] = bare_prompt.group(1)
        if 'f' not in p and p.get('id'):
            p['f'] = p['id'].lower()

# ── Fallback: bare search: field (flat media panels) ──
if 'source_search' not in p:
    bare_search = re.search(r'(?<!\w)search:\s*"((?:[^"\\]|\\.)*)"', ptext)
    if bare_search:
...
return f”{significant[0][:6]}*{significant[-1][:6]}”
elif significant:
